...
bot = commands.Bot(command_prefix=str(config.prefix), case_insensitive=True,heartbeat_timeout=300)
...
def run_client(token):
	...
	while True:
		...
		time.sleep(60)
		bot = commands.Bot(command_prefix=get_prefix, case_insensitive=True,heartbeat_timeout=300)
		bot.remove_command("help")
		#if bot.is_closed():
			#loop.run_until_complete(bot.connect())
